...
public class fifo {
	...
	public fifo(ArrayList<ArrayList<String>> inst, int tasks, int resources, int[] resource_units){
		tasknum = tasks;
		tasktemp = 0;
		...
		while(terminated.size() != inst.size()){
			...
			for(int j = 0; j < blocksize; j++){
				...
				for(int i = 0; i < resourcenum; i++){
					if(need[temp][i] > avail[i]){
						//System.out.println("need: " + need[temp][i] + " avail: " + avail[i]);
						terminate = true;
					}
				}
				if (terminate && (blockQueue.size() + terminated.size()) == inst.size()){
					waiting[temp]++;
				...
				if(delay[i] == 0){
					if(blockQueue.contains(i)){
						...
						if(need[Integer.parseInt(command[1])-1][Integer.parseInt(command[3])-1] > claims[Integer.parseInt(command[1])-1][Integer.parseInt(command[3])-1]){
							...
							if(avail[Integer.parseInt(command[3])-1] < Integer.parseInt(command[4])){
								blockQueue.add(i);
